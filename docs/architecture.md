...
- tessellation-free hidden-face rim regions, including dihedral-aware opening widths for embedded planar surfaces, independently closed normal-offset pieces for immersed or folded surfaces, uninterrupted one-sided immersed source-edge sheets, width limits, and source-edge provenance;
...
outer and hole cycles. Embedded planar surfaces use equal-offset face-plane intersections for their
shared inner shell. Immersed or folded surfaces use independently closed local-normal pieces, because
their crossing sheets do not define a bounded shared thickness miter. These buffers feed WebGL and the
table-shadow pass. STL is a separate Wasm consumer of authoritative geometry and presentation settings;
OpenSCAD is a polygonal JS consumer.
