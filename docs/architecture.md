...
- tessellation-free hidden-face rim regions, including dihedral-aware opening widths, one-sided higher-winding strips, flat zero/nonzero covers, bounded source-vertex joins, width limits, and source-edge provenance;
...
Hidden-face buffers triangulate the worker's polygonal rim regions, build opening walls on final
outer and hole cycles, and add the core's flat zero/nonzero source covers. Their inner positions use
the same equal-offset face-plane intersections as export. These buffers feed WebGL and the table-shadow pass. STL is a separate Wasm
