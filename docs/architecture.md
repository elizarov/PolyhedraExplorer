...
- cached coplanar face-overlap partitions, including source-face membership and rim-coverage masks,
  shared by WebGL and face/edge popup figures; see [Self-intersecting polyhedra](self-intersections.md);
- shared full-point-group face-orbit enumeration and progress-reporting exact-cover faceting: Greatened applies it to the polar dual before reciprocal reconstruction and source-plane alignment; experimental Faceted applies it directly to the source positions. Both validate closure and use geometry-only result ordering; direct faceting deduplicates complete face circuits, not wireframes;
- normalized face-plane-arrangement construction for Stellated, including orbit-representative bounded diagrams expanded by geometric symmetries, spatially indexed intersections, symmetric circuit candidates, lazily materialized main-line cell-power boundaries, independent manifold compound circuits, phased progress, bounded LRU reuse, and discrete Result metadata shared with Greatened and Faceted. Every cached result owns lazily computed geometry-contract analysis, full point-group and F/E/V orbit classification, core-derived orbit-action tags, and most recent presentation-rim geometry, so returning to it does not repeat that post-processing;
...
finite-coordinate, manifold-edge, per-component vertex-fan, triangle-orientation, or surface-intersection validation before
it can reach WebGL.
...
- Every displayed polyhedron satisfies at least the renderable-immersion contract. Resolved output
  and every successful STL satisfy the stronger embedded-boundary contract, retaining all physical
  components. Simple non-planar faces remain valid through their shared triangulation.
