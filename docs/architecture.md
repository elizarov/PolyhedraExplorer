...
The mesh and transform validity contract for concave faces, non-planar faces, compounds, and
intersections is specified in [Non-convex geometry](non-convex.md). Every face consumer uses the
model's same deterministic ear-clipped indices; transform validation therefore checks the exact
