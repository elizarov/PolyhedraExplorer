...
- Transform chains containing the primitive Truncated, Rectified, Dual, Snub, Chamfered, `Canonical` (the canonicalization operation), and topology-valid Drop operations. None removes an existing stage. The transform popup lists fixed primitives under Transform, combinations under Macro, and, when available, one Drop face/edge/vertex entry per supported target family under a final Orbit-targeted section, in the UI's standard F/E/V order.
- A newly added orbit-targeted operation selects the first valid orbit and is displayed with its concrete target, such as `Drop A`. While it is the last chain item, compact up/down controls to its right cycle and wrap through the currently valid orbits of the same element family.
...
- Optional catalog replacement: when a completed transform chain is orientation-preservingly equivalent to a built-in seed, a suggestion for the proper handed variant appears to the right of the chain. The current exploratory state remains unchanged until the suggestion is clicked.
