...
- Snub, Propeller, Whirl, Gyro, and the four chiral seed types have a prime-tagged alternate handedness. A flip button appears immediately to the right of a chiral item only while that item is last in the applied chain; flipped names and URL tags end in `'`. Quinto is achiral and has no flip control.
...
- Background WasmGC execution for manipulation algorithms, with live per-stage convergence progress attached to the transform pill currently consuming CPU and cancellation when a newer state supersedes it. The central loading status represents only Wasm module loading and disappears before transform progress begins.
