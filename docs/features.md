...
- Transform chains containing the primitive Truncated, Rectified, Dual, Snub, Chamfered, `Canonical` (the canonicalization operation), and topology-valid Drop operations. None removes an existing stage. The transform popup lists fixed primitives under Transform, combinations under Macro, and, when available, one Drop edge/vertex/face entry per supported target family under a final Orbit-targeted section.
