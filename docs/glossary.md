...
| Immersed surface | A connected abstract two-manifold whose geometric face realization may cross itself. Rendering consumes each source face's precomputed nonzero-winding presentation cells; the authoritative source topology remains unchanged. |
...
| Primitive transform | A directly executable transform. The fixed primitive choices are Truncated, Rectified, Dual, Snub, Propeller, Whirl, Quinto, Chamfered, Canonical, Greatened, Stellated, and Resolved; topology-dependent Drop, Kis face, Truncate vertex, Rectify vertex, and Radial vertex operations are also primitive. |
...
| Greatening | A face-plane-constellation transform that selects a strictly larger circuit while retaining each face's cyclic type. It is serialized as `G`; catalog recognition only verifies its output afterward. |
| Stellation | A face-plane-arrangement transform that adds successive complete cell-power strata from the convex core, constructs a proper or immersed boundary, and orders supported main-line results from closest to farthest. It is serialized as `S`; compounds and invalid surfaces are discarded. |
...
| Face rim | Polygonal material retained around the authoritative boundary of a hidden face. Simple faces use a uniform inset with bounded bevels at sharp corners; immersed faces union full-width exterior strips and half-width internal strips over their resolved nonzero-winding fill, preserving visible star crossings. The core supplies outer/hole cycles and provenance, while each consumer triangulates them when needed. |
