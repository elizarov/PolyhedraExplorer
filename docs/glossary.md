...
| Resolved physical boundary | The embedded polygonal boundary between zero- and nonzero-winding material selected by Resolve. Its F/E/V topology includes intersection-created elements and can differ from the immersed source. |
| Resolve | The primitive `R` transform that corefines an immersed surface, selects its nonzero-winding physical boundary, and assigns final rotation-orbit kinds while retaining many-to-many source provenance. |
...
| Primitive transform | A directly executable transform. The fixed primitive choices are Truncated, Rectified, Dual, Snub, Propeller, Whirl, Quinto, Chamfered, Canonical, Greatened, Stellated, and Resolve; topology-dependent Drop, Kis face, Truncate vertex, Rectify vertex, and Radial vertex operations are also primitive. |
...
| Stellation | A face-plane-constellation transform that continues source edge lines into a strictly larger circuit and may change face winding. It is serialized as `S`; disconnected or non-manifold candidates are discarded. |
