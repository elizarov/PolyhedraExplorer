...
| [Seed](seeds.md) | A built-in starting polyhedron selected before transforms. There are 35 fixed types (31 convex and four Kepler-Poinsot), four additional prime-tagged chiral representations, and four families with `n = 3..100`. |
| Kepler-Poinsot solid | One of the four regular non-convex polyhedra. Its classical regular faces form an immersed arrangement; the application stores its non-intersecting resolved physical boundary. |
| Immersed surface | A mathematical face arrangement allowed to pass through itself. Classical star-polyhedron faces are immersed, but an immersed mesh is never sent to rendering or export. |
| Resolved physical boundary | The embedded triangular boundary between zero and nonzero winding regions of a classical star-face arrangement. Its F/E/V counts include face-intersection cells and therefore differ from the abstract Kepler-Poinsot counts. |
...
| Primitive transform | A directly executable transform. The fixed primitive choices are Truncated, Rectified, Dual, Snub, Propeller, Whirl, Quinto, Chamfered, Canonical, Greatened, and Stellated; topology-dependent Drop, Kis face, Truncate vertex, and Rectify vertex operations are also primitive. |
| Greatening | A regular-star transform that changes a recognized Dodecahedron, Icosahedron, or Small stellated dodecahedron to its supported great realization. It is serialized as `G`. |
| Stellation | Continuation of a recognized regular face-plane arrangement to a supported star form. The primitive `S` maps Dodecahedron to Small stellated dodecahedron and Great dodecahedron to Great stellated dodecahedron. |
